...
+ (BOOL)insertFile:(FileModel *)fileModel
{
    ...
    [DBQueueShare inDatabase:^(FMDatabase *db) {
        ...
        if (url) {
            ret = [db executeUpdate:@"UPDATE DOWNLOADFILE SET LocalFilePath = ?,  FileName = ?, CreateDate = ?, fileState = ?, tempFileName = ?, tempPath = ?, resumeData = ?, FileSize = ?, DownloadSize = ?, teacherName = ?, teacherIntro = ?, courseTime = ?, courseIntro = ?, tpic = ? WHERE fileUrl = ?",fileModel.filePath,fileModel.fileName,fileModel.fileDownedTime,@(fileModel.fileState),fileModel.tempFileName,fileModel.tempPath,fileModel.resumeData,fileModel.fileSize,fileModel.fileReceivedSize,fileModel.teacherName, fileModel.teacherIntro, fileModel.courseTime, fileModel.courseIntro, fileModel.tpic, fileModel.fileUrl];
        } else {
            ret = [db executeUpdate:@"INSERT INTO DOWNLOADFILE(LocalFilePath, FileSize, DownloadSize, FileName, fileUrl, CreateDate, fileState,tempFileName,tempPath,resumeData,teacherName, teacherIntro, courseTime, courseIntro, tpic) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",fileModel.filePath,fileModel.fileSize,fileModel.fileReceivedSize,fileModel.fileName,fileModel.fileUrl,fileModel.fileDownedTime,@(fileModel.fileState),fileModel.tempFileName,fileModel.tempPath,fileModel.resumeData,fileModel.teacherName, fileModel.teacherIntro, fileModel.courseTime, fileModel.courseIntro, fileModel.tpic];
        }
    }];
    return ret;
}
...
+ (FileModel *)fileModelWithResult:(FMResultSet *)result
{
    ...
    //courseIntro;
    //tpic;
    model.teacherName = [result stringForColumn:@"teacherName"];
    model.teacherIntro = [result stringForColumn:@"teacherIntro"];
    model.courseTime = [result stringForColumn:@"courseTime"];
    model.courseIntro = [result stringForColumn:@"courseIntro"];
    model.tpic = [result stringForColumn:@"tpic"];
